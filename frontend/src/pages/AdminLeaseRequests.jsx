// src/pages/AdminLeaseRequests.jsx
import React, { useEffect, useState } from 'react';
import { useNavigate } from 'react-router-dom';
import api from '../services/api';

export default function AdminLeaseRequests() {
  ...
  return (
    <div style={styles.page}>
      <nav style={styles.navbar}>
        <div style={styles.navLeft}>📑 Admin Lease Requests</div>
        <div style={styles.navRight}>
          <button style={styles.navButton} onClick={() => navigate('/admin-panel')}>Dashboard</button>
      ...
      <div style={styles.container}>
        <h2 style={styles.heading}>All Lease Requests</h2>
        ...
        <div style={styles.tabs}>
          {['all', 'pending', 'approved', 'rejected'].map(tab => (
            ...
            >
              ...
              ({requests.filter(r => r.status === tab || tab === 'all').length})
            </button>
          ))}
        </div>
